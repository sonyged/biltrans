...
describe('translate wait', function() {
  ...
  it('should be wait in bson', function() {
    assert.deepEqual(trans.translate(), new Buffer([
      0x1d, 0x00, 0x00, 0x00, 0x10, 0x6e, 0x61, 0x6d,
      0x65, 0x00, 0x06, 0x00, 0x00, 0x00, 0x01, 0x73,
      0x65, 0x63, 0x73, 0x00, 0x33, 0x33, 0x33, 0x33,
      0x33, 0x33, 0xe3, 0x3f, 0x00
    ]));
  });
});
...
describe('execute port-settings', function() {
  ...
  it('should execute port-settings', function() {
    ...
    assert.deepEqual(output, [
      "port-init 24 push-button",
      "port-init 25 push-button",
      "port-init 0 multi-led",
      "port-init 28 light-sensor",
      "port-init 29 ir-photo-reflector",
      ""
    ]);
  });
});
